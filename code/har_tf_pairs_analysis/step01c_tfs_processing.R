...
for (species in c("human", "chimp")) {
  result_dir <- check_dir(
    file.path("results/har_tf", species)
  )
...
human_stats <- fread(file.path("results/har_tf/human/statistics.csv"))
human_stats[, Species := "Human"]

chimp_stats <- fread(file.path("results/har_tf/chimp/statistics.csv"))
