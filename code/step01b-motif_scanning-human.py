...
def create_har_mapping():
    ...
    """
    har_df = pd.read_csv("results/step01-har_tf/human/har_coords.csv")

    har_mapping = {}
    for _, row in har_df.iterrows():
        # For new HAR format (HARsv2_xxxx), use har directly
        if "HARsv2_" in str(row["har"]):
            har_name = row["har"]
            har_mapping[har_name] = har_name
        else:
            # For old HAR format (ZOOHAR.xxx), use coordinate mapping
...
def process_database_hits(database_name, hits):
    ...
    result_dir = "results/step01-har_tf/human"
...
def main():
    ...
    har_seqs_file = os.path.join(result_dir, "har_seqs_hg38.fa")
...
subprocess.run(["bash", "./code/functions/download_motif_files.sh"])
