...
def load_network_data(file_path: str):
    ...
    try:
        ...
        print(f"{get_colored_time()} {Fore.RED}Error loading data from {file_path}: {str(e)}{Style.RESET_ALL}")
        print(f"{get_colored_time()} Please ensure you have run the network processing script first:")
...
def load_har_tf_data(
    ...
    if file_path is None:
        print(f"{get_colored_time()} {Fore.YELLOW}No HAR-TF data file provided.{Style.RESET_ALL}")
        print(f"{get_colored_time()} {Fore.YELLOW}HAR-TF data will not be included in the Sankey diagram.{Style.RESET_ALL}")
        return None
    try:
        ...
        print(f"{get_colored_time()} {Fore.RED}Error loading HAR-TF data from {file_path}: {str(e)}{Style.RESET_ALL}")
        sys.exit(1)
    ...
    with tqdm(total=7, desc="Generating visualization", ascii=True) as pbar:
        ...
        if len(flow_data) > 0:
            ...
            try:
                ...
                print(f"{get_colored_time()} {Fore.YELLOW}Warning: Could not save PDF version: {str(e)}{Style.RESET_ALL}")
                print(f"{get_colored_time()} HTML version is still available.")
            pbar.update(1)

            print(f"{get_colored_time()} {Fore.GREEN}Visualization completed successfully.{Style.RESET_ALL}")
            return fig
        else:
            print(f"{get_colored_time()} {Fore.RED}No data found for the specified parameters.{Style.RESET_ALL}")
            return None
...
if __name__ == "__main__":
    try:
        ...
        print(f"{get_colored_time()} Visualization files have been generated in {output_path}")

    except Exception as e:
        print(f"{get_colored_time()} {Fore.RED}\nError creating Sankey diagram: {str(e)}{Style.RESET_ALL}")
        import traceback
