...
not_built_csv <- file.path(
  res_dir, "not_built_networks_summary.csv"
)
  ...
  if (!file.exists(network_file)) {
    ...
    for (ct in cell_types) {
      ...
      network_file_ct <- file.path(
        res_dir_rds, paste0(file_base, "_", ct, ".rds")
      )
      if (!file.exists(network_file_ct)) {
        network_list[[ct]] <- tryCatch(
          {
            ...
            if (is.null(net) || nrow(net) == 0) {
              log_message(
                "Empty network result for {.val {ct}} - {.val {file_name}}"
              )
              not_built_networks <<- rbind(
                not_built_networks, data.frame(
                  BrainRegion = brain_region,
        ...
        log_message("Reading existing network from {.val {network_file_ct}}...")
        network_list[[ct]] <- readRDS(network_file_ct)
      }
    }
